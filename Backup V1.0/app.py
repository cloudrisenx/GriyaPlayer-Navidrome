...
def player():
    ...
    # Logika tendang (logout) jika mendeteksi halaman di-refresh (bukan load pertama setelah login)
    if not session.pop('just_logged_in', False):
        session.clear()
        return redirect(url_for('index'))
        
    return render_template('player.html', stream_port=get_user_config()['stream_port'], username=session['username'])
...
def api_playlist_create():
    ...
    data = read_playlists()
    if not any(p.get("name") == name for p in data.get("playlists", [])):
...
def api_playlist_remove():
    ...
    data = read_playlists()
    for pl in data.get("playlists", []):
        if pl.get("name") == playlist_name and song_id in pl.get("songs", []):
...
def trigger_interrupt():
    ...
    if filename:
        try:
            # Gunakan c.pause(1) agar judul dan posisi lagu tidak hilang saat interupsi selesai
            c = get_mpd()
            print(f"[{session['username']}] MPD: Mengirim pause(1)")
            c.pause(1) 
            c.disconnect()
        except Exception as e: 
...
def mpd_cmd():
    ...
    try:
        ...
        if cmd == "play":
            ...
            if st == "play": c.pause(1)
            elif st == "pause": c.pause(0)
            else: c.play()
        elif cmd == "resume_playback":
            st = c.status().get('state')
            if st == "pause": c.pause(0)
            elif st == "stop": c.play()
        elif cmd == "next": c.next()
        elif cmd == "prev": c.previous()
        elif cmd == "shuffle_on":
            c.random(1)
