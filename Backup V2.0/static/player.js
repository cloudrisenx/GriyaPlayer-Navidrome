...
function ensureSnapwebButton() {
    if (!document.getElementById('btn-snapweb-ui')) {
        ...
        if (modeBtn) {
            const btnHtml = `<button id="btn-snapweb-ui" style="margin-left: 8px; background: rgba(76, 175, 80, 0.1); border: 1px solid rgba(76, 175, 80, 0.3); color: #4caf50; border-radius: 12px; padding: 4px 8px; font-size: 11px; cursor: pointer; display: inline-flex; align-items: center; gap: 4px; transition: 0.2s;" onclick="openSnapwebModal(event)" title="Buka Player Latar Belakang"><i class='bx bx-podcast'></i> Audio Latar</button>`;
            modeBtn.insertAdjacentHTML('afterend', btnHtml);
        }
    }
}
        ...
        <div class="modal-content" onclick="event.stopPropagation()" style="width: 320px; text-align: center;">
            ...
            <div style="display: flex; flex-direction: column; gap: 8px; max-height: 300px; overflow-y: auto; text-align: left;">
                ${Object.entries({
                    ...
                    8003: 'Hugo', 8004: 'Khayangan', 8005: 'Obakso',
