...
const MPD_STREAM = window.location.protocol + "//" + window.location.hostname + ":" + window.STREAM_PORT; 
...
function fetchMissingMetadata(ids) {
    ...
    ids.forEach(id => {
        if (id && !songCache[id] && !isFetchingCache[id]) {
            ...
            .then(r => {
                if (r.status === 401) { window.location.replace(window.BASE_URL + '/'); return {}; }
                return r.json();
            })
            .then(d => {
...
async function doLogout() {
    await fetch(window.BASE_URL + '/logout', { method: 'POST' });
...
async function loadSidebarPlaylists() {
    try {
        ...
        if (fetchRes.status === 401) { window.location.replace(window.BASE_URL + '/'); return; }
